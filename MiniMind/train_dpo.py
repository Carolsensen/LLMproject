...
def train_epoch(epoch, wandb, metrics_writer, metrics_file):
    ...
    for step, batch in enumerate(train_loader):
        ...
        # 6. 记录指标并写入CSV（仅主进程执行）
        # 新增：计算全局步数（关键修正）
        current_step = epoch * iter_per_epoch + step  # 全局步数 = 已完成epoch数*每epoch步数 + 当前step
        if (not ddp or dist.get_rank() == 0):
            # 提取当前损失值（反归一化，还原真实损失）
            current_loss = loss.item() * args.accumulation_steps
            # 写入CSV：全局步数、损失、学习率
            metrics_writer.writerow([current_step, current_loss, lr])
            # 强制刷新缓冲区，确保数据实时写入文件
            metrics_file.flush()
...
if __name__ == "__main__":
    ...
    if not ddp or dist.get_rank() == 0:
        ...
        metrics_writer.writerow(["step", "loss", "lr"])  # 表头：全局步数、损失、学习率
    ...
    for epoch in range(args.epochs):
        train_epoch(epoch, wandb, metrics_writer, metrics_file)
